...
var AbstractConnectionManager = require( '../abstract/connection-manager' ),
	...
	parserStore = require( '../parserStore' )( 'spatialite' );
...
ConnectionManager = function( dialect, sequelize ) {
	...
	try {
		this.lib = require( sequelize.config.dialectModulePath || 'spatialite' ).verbose();
	} catch ( err ) {
		throw new Error( 'Please install spatialite package manually' );
	}
...
ConnectionManager.prototype.getConnection = function( options ) {
	...
	options.inMemory = ( ( self.sequelize.options.storage || self.sequelize.options.host ||
		':memory:' ) === ':memory:' ) ? 1 : 0;
	...
	if ( self.connections[ options.inMemory || options.uuid ] ) {
		...
		return self.connections[ options.inMemory || options.uuid ];
	}
	...
	return new Promise( function( resolve, reject ) {
		self.connections[ options.inMemory || options.uuid ] = new self.lib.Database(
			...
			function( err ) {
				if ( err ) {
					if ( err.code === 'SQLITE_CANTOPEN' ) return reject( new sequelizeErrors.ConnectionError(
						err ) );
