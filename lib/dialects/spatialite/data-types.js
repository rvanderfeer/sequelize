...
module.exports = function( BaseTypes ) {
	var warn = BaseTypes.ABSTRACT.warn.bind( undefined, 'https://www.spatialite.org/datatype3.html' );

	BaseTypes.DATE.types.spatialite = [ 'DATETIME' ];
	BaseTypes.STRING.types.spatialite = [ 'VARCHAR', 'VARCHAR BINARY' ];
	BaseTypes.CHAR.types.spatialite = [ 'CHAR', 'CHAR BINARY' ];
	BaseTypes.TEXT.types.spatialite = [ 'TEXT' ];
	BaseTypes.INTEGER.types.spatialite = [ 'INTEGER' ];
	BaseTypes.BIGINT.types.spatialite = [ 'BIGINT' ];
	BaseTypes.FLOAT.types.spatialite = [ 'FLOAT' ];
	BaseTypes.TIME.types.spatialite = [ 'TIME' ];
	BaseTypes.DATEONLY.types.spatialite = [ 'DATE' ];
	BaseTypes.BOOLEAN.types.spatialite = [ 'TINYINT' ];
	BaseTypes.BLOB.types.spatialite = [ 'TINYBLOB', 'BLOB', 'LONGBLOB' ];
	BaseTypes.DECIMAL.types.spatialite = [ 'DECIMAL' ];
	BaseTypes.UUID.types.spatialite = [ 'UUID' ];
	BaseTypes.ENUM.types.spatialite = false;
	BaseTypes.REAL.types.spatialite = [ 'REAL' ];
	BaseTypes.DOUBLE.types.spatialite = [ 'DOUBLE PRECISION' ];
	BaseTypes.GEOMETRY.types.spatialite = [ 'GEOMETRY' ];
	...
	TEXT.prototype.toSql = function() {
		if ( this._length ) {
			warn( 'Spatialite does not support TEXT with options. Plain `TEXT` will be used instead.' );
	...
	BaseTypes.GEOMETRY.types.spatialite = {
		oids: [],
		array_oids: []
	};
