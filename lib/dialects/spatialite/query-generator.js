...
var QueryGenerator = {
			...
			if ( attributes.hasOwnProperty( attr ) ) {
				...
				if ( Utils._.includes( dataType, 'PRIMARY KEY' ) ) {
					if ( Utils._.includes( dataType, 'INTEGER' ) ) { // Only INTEGER is allowed for primary key, see https://github.com/sequelize/sequelize/issues/969 (no lenght, unsigned etc)
						dataTypeString = containsAutoIncrement ? 'INTEGER PRIMARY KEY AUTOINCREMENT' :
							'INTEGER PRIMARY KEY';
					}
	...
	upsertQuery: function( tableName, insertValues, updateValues, where, rawAttributes, options ) {
		...
		var sql = this.insertQuery( tableName, insertValues, rawAttributes, options ) + ' ' + this.updateQuery(
			tableName, updateValues, where, options, rawAttributes );
	...
	updateQuery: function( tableName, attrValueHash, where, options, attributes ) {
		...
		for ( var key in attrValueHash ) {
			...
			values.push( this.quoteIdentifier( key ) + '=' + this.escape( value, ( modelAttributeMap &&
				modelAttributeMap[ key ] || undefined ), {
				context: 'UPDATE'
			} ) );
		}
	...
	describeTableQuery: function( tableName, schema, schemaDelimiter ) {
		...
		return Utils._.template( sql )( {
			tableName: this.quoteTable( this.addSchema( table ) )
		} );
	},

	removeColumnQuery: function( tableName, attributes ) {
		var backupTableName, query;
		...
		query = [
			this.createTableQuery( backupTableName, attributes ).replace( 'CREATE TABLE',
	...
	renameColumnQuery: function( tableName, attrNameBefore, attrNameAfter, attributes ) {
		...
		return Utils._.template( query )( {
			...
			attributeNamesImport: Utils._.keys( attributes ).map( function( attr ) {
				return ( attrNameAfter === attr ) ? this.quoteIdentifier( attrNameBefore ) + ' AS ' +
	...
	replaceBooleanDefaults: function( sql ) {
		return sql.replace( /DEFAULT '?false'?/g, 'DEFAULT 0' ).replace( /DEFAULT '?true'?/g,
			'DEFAULT 1' );
	},
